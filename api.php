...
if (preg_match('#^/games/(\d+)/move$#', $path_info, $m) && $method === 'POST') {
    ...
    $opp = ($current_player === $board['player1_id'])
        ? $board['player2_id']
        : $board['player1_id'];

    $round = check_end_of_round_or_game(
        $game_data,
        $res['board_state'],
        $current_player,
        $opp
    ...
    $st = $mysqli->prepare(
        "UPDATE games SET board_state=?, current_turn=?, status=? WHERE game_id=?"
    );
